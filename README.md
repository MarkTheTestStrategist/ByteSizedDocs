# ByteSizedDocs

Documents that include cheatsheets, how to guides and anything else of relevance for software testing, but would also be of use for developers too.

## Index

### API

- [API 400 vs 500](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/API/API_400_vs_500.md)
- [API Parameters - Sorting](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/API/API_Parameters-Sorting.md)
- [API Testing Complete Test Suites CSV files in a Zip file](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/API_Testing_-_Complete_Test_Suites.zip)
- [API Testing CheatSheet](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/API/API_Testing_CheatSheet.md)
- [API Testing Principles](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/API/API_Testing_Principles.md)
- [API Troubleshooting](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/API/API_Troubleshooting.md)
- [JWT Token Access - Property explanations](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/API/JWT_Access_Tokens.md)
- [Understanding Json Properties](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/API/Understanding-Json-Properties.md)
  
### Datasets

- [UK_Phone_Numbers](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/UK_Phone_numbers.md)
- [UK Unusual Postcodes](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/Unusual_UK_Postcodes.md)

### LimitBreakers

- [ISO Standards](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/LimitBreakers/ISO_Standards.md)
- [Phone Numbers Boundaries](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/LimitBreakers/Phone_Numbers_Boundaries.md)

### ServiceBus

- [Service-bus-Tests-CheatSheet CSV](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/Service-Bus-Tests-CheatSheet.csv)
- [Service Bus CheatSheet](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/ServiceBusCheatSheet.md)

### Articles Worthy of Note

- [Test Websites for practicing automation](https://github.com/MarkTheTestStrategist/ByteSizedDocs/blob/main/UsefulLinks/TestWebsites.md)
